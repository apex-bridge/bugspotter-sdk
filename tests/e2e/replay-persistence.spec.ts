...
test.describe('Replay Persistence — Real Browser', () => {
  ...
  test('full reload restores prior-session events from IDB', async ({
    ...
    // Generate replay events the rrweb recorder will buffer.
    await page.click('#btn-a');
    await page.click('#btn-b');
    // Give rrweb's emit pipeline a tick to land events in the buffer.
    ...
    // browser's IDB teardown on close).
    await dispatchVisibilityChange(page, 'hidden');
    await page.waitForTimeout(200);

    // Confirm IDB has events before reload.
    const beforeCount = await readReplayCount(page, dbName);
    ...
    // Give restore() its await microtask + addBatch.
    await page.waitForTimeout(300);

    // After restore, IDB should be empty (consumed by readAndClear)
    // but the SDK's in-memory replay buffer should hold the events.
    const afterClearCount = await readReplayCount(page, dbName);
  ...
  test('two tabs in same context have isolated dbNames via sessionStorage tab id', async ({
    ...
    const tab1Count = await readReplayCount(page1, tab1Db);
  ...
  test('visibilitychange to hidden triggers IDB flush; visible does not', async ({
    ...
    // Positive case: hidden → flush lands.
    await dispatchVisibilityChange(page, 'hidden');
    await page.waitForTimeout(200);
    const afterHidden = await readReplayCount(page, dbName);
    ...
    // Allow a small delta for background events; reject any
    // duplication of the already-flushed set.
  ...
  test('back-forward navigation does not duplicate events on next flush', async ({
    ...
    // memory; otherwise this is a cold reload.
    await page.goBack();
    await page.waitForTimeout(300);
    ...
    // reset (slice 3) should prevent duplication regardless of
    // which path the browser took.
    await page.click('#btn-b');
    await page.waitForTimeout(150);
    await dispatchVisibilityChange(page, 'hidden');
    await page.waitForTimeout(200);
